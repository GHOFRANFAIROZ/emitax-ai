...
DEC = {"facility": "A", "unit": "U1", "ym": "2025-01", "trust_score": 55,
       "action": "request_docs", "fired_checks": "measurement", "reason": "Beyan ölçümün altında"}
...
def test_tampering_breaks_hash():
    ...
    assert verify_record(rec2) is False          # her değişiklik tespit edilir


def test_onchain_has_no_raw_values():
    rec = build_record(DEC, DECL, MEAS, CFG, verified_at=VA)
    oc = onchain_payload(rec)
    assert "declared" not in oc and "measured_summary" not in oc   # zincirde ham değer yok
