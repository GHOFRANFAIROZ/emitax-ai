...
torch = pytest.importorskip("torch")   # torch yoksa yerelde atlanır, Colab'da çalışır
...
def test_model_shape_roundtrip():
    ...
    y = m(x)
    assert tuple(y.shape) == (5, 24, 4)          # çıktı = giriş, şekil olarak
...
def test_train_reduces_loss_and_error_shape():
    ...
    assert hist[-1] <= hist[0] * 1.5             # patlamıyor
